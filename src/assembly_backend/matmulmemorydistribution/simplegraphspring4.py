...
class DoubleBuffer:
    """
    Double buffer for safe data exchange between physics and renderer threads.
    """
    def __init__(self, initial_data):
        self.buffers = [initial_data, initial_data.copy()]
        self.read_idx = 0
        self.write_idx = 1
    ...
    def write(self, data):
        with self.lock:
            self.buffers[self.write_idx] = data.copy()

    def read(self):
        with self.lock:
            return self.buffers[self.read_idx].copy()
...
class PhysicsEngine:
    ...
    def step(self, dt):
        positions, velocities = self.net.get_state()
        ...
        new_positions, new_velocities = self.net.step(dt)
        return new_positions, new_velocities

def physics_worker(double_buffer, stop_event, net, positions, velocities, *args):
    """
...
def render_worker(double_buffer, stop_event, *args):
    """
    Rendering thread.
    """
    while not stop_event.is_set():
        ...
        # ...render using positions...
        # (Insert rendering logic here, e.g., update_vbo, draw calls, etc.)
        # Optionally sleep to control frame rate
        time.sleep(1.0 / FPS)

...
if __name__ == "__main__":
    ...
    model = TorchParametricSolver([A_labels,B_labels,C_labels,D_labels]).to(device)
    ...
    def image_sequence():
        ...
        for step in range(args.steps):
            ...
            optimizer.zero_grad()
            inter_pen, intra_pen, mat_ops, addr_map = model(use_sinkhorn=use_sinkhorn, sinkhorn_temp=sinkhorn_temp)
            loss = inter_pen + intra_pen
            if loss.item() < best_loss:
                best_loss = loss.item()
                best_state = {
                    'mem_logits': model.mem_logits.detach().clone(),
                    'op_params': model.op_params.detach().clone(),
                    'use_sinkhorn': use_sinkhorn,
                    'sinkhorn_temp': sinkhorn_temp
                }
                # Cache best mat_ops and addr_map only when new best is found
                ...
                def to_cpu_mat_ops(mat_ops):
                    ...
                    return out
                best_state['cpu_best_addr_map'] = to_cpu_addr_map(addr_map)
                best_state['cpu_best_mat_ops'] = to_cpu_mat_ops(mat_ops)
                # Also cache the best image and the data needed to recreate it
                temporal_usage_best = {label: idx for idx, (op, (dst_label,_), _) in enumerate(mat_ops)
                                      for label in [dst_label] + [src for src,_ in _]}
                max_time_best = max(1, max(temporal_usage_best.values(), default=0))
                best_state['temporal_usage'] = temporal_usage_best
                best_state['max_time'] = max_time_best
                best_state['img_best'] = generate_overlay_image(
                    best_state['cpu_best_addr_map'],
                    best_state['cpu_best_mat_ops'],
            ...
            if step % 200 == 0:
                ...
                # Use cached best visualization and data
                if best_state is not None and 'img_best' in best_state:
                    cpu_best_addr_map = best_state['cpu_best_addr_map']
                    cpu_best_mat_ops = best_state['cpu_best_mat_ops']
                    img_best = best_state['img_best']
                else:
                    cpu_best_addr_map = None
                    cpu_best_mat_ops = None
                    img_best = None
                ...
                if display_mode == 'side_by_side':
                    ...
                    # img_best is already set from cache above
                elif display_mode == 'current_only' or img_best is None:
                    img_current = generate_overlay_image(
                        cpu_addr_map, cpu_mat_ops, logical_order, D_labels,
                        hsv_config=hsv_config,
                    ...
                    # img_best is already set from cache above
                    pass
                print(f"Step {step:4d} Loss={loss.item():.2f} Best={best_loss:.2f} LR={optimizer.param_groups[0]['lr']:.4f} Sinkhorn={use_sinkhorn}")
                yield (img_current, img_best)
                # (Optional: per-op visualization, unchanged)
                for op in mat_ops:
                    break
                    img_one_op_current = generate_overlay_image(
                        cpu_addr_map, [op], logical_order, D_labels,
                        hsv_config=hsv_config,
                        border_hsv_config=border_hsv_config,
                        shown_groups=shown_groups,
                        separate_matrices=True,
                    ...
                    img_one_op_best = generate_overlay_image(
                        cpu_best_addr_map, [op], logical_order, D_labels,
                        hsv_config=hsv_config,
                        border_hsv_config=border_hsv_config,
                        shown_groups=shown_groups,
                        separate_matrices=False,
                    ...
                    if img_one_op_best is not None and img_one_op_current.shape[0] != img_one_op_best.shape[0]:
                        min_h = min(img_one_op_current.shape[0], img_one_op_best.shape[0])
                        img_one_op_current = img_one_op_current[:min_h,:,:]
                        img_one_op_best = img_one_op_best[:min_h,:,:]
                    combined_img_one_op = np.concatenate((img_one_op_current, img_one_op_best), axis=1) if img_one_op_best is not None else img_one_op_current
                    yield (img_one_op_current, img_one_op_best)

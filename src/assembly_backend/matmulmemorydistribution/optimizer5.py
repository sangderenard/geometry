...
import pandas as pd
from graph_express import extract_operations_and_symbols
...
import torch
import pandas as pd
from types import Node, NodeSet, Operation, Edge
...
class Diagnostics:
    ...
    def global_node_idx_to_set_location(self, global_idx):
        set_locations = []
        input_sets = self.data.get('input_sets', [])
        output_sets = self.data.get('output_sets', [])
        input_node_set_lengths = [len(s) for s in input_sets]
        output_node_set_lengths = [len(s) for s in output_sets]
        if isinstance(global_idx, int):
            for i, length in enumerate(input_node_set_lengths):
                if global_idx < length:
                    set_locations = [i]
        elif isinstance(global_idx, torch.Tensor):
            
            for idx in global_idx:
                idx = idx.item() if isinstance(idx, torch.Tensor) else idx
                for i, length in enumerate(input_node_set_lengths):
                    if idx < length:
                        set_locations.append((i, idx))
                        break
                    idx -= length
        return_data = []
        for set_location in set_locations:
            node_set = self.data['node_sets'][set_location[0]]
            return_data.append(self.get_set_location(node_set.id_from_idx(set_location[1])))
        return return_data


        
    def register_memory_map(self, memory_map):
        """
        Take in memory logits and register them as a mapping."""

        if self.data is None:
            self._build_ops_df()

        self.data['memory_map'] = memory_map

    def transform_by_memory_map(self, memory_map=None, placement_ordered_set=None):
        if memory_map is None:
            memory_map = self.data.get('memory_map', None)
        if placement_ordered_set is None or memory_map is None:
            return None
        return placement_ordered_set.index_select(0, torch.tensor(memory_map, dtype=torch.long, device=device))

    def obtain_nodes_and_edges(self):
        node_ids = torch.arange(len((self.data.get('input_sets', []) + self.data.get('output_sets', [])).flatten()), 
                              dtype=torch.long, device=device)
        memory_locations = self.get_memory_location_from_node_id(node_ids)
        nodes = []
        for node_id in node_ids.tolist():
            node = Node(
                id=node_id,
                location_in_set=self.get_set_location_from_node_id(node_id),
                location_in_memory=memory_locations[node_id],
                readwrite=self.data.get('input_masks', [])[node_id] if node_id < len(self.data.get('input_sets', [])) else None,
                exclusionary=True
            )
            nodes.append(node)
            
        edges = []
        for op_set in self.data.get('operation_sets', []):
            for (op, (reads, writes)) in op_set:
                edges.append( Operation(reads, len(reads), writes, len( writes), op) )
        return nodes, edges

    def get_node_sets(self, set_data):
        """
        Extract node sets from the provided set_data.
        """
        node_sets = []
        for set_name, (n, m, k) in set_data:
            node_set = NodeSet(n, m, k)
            node_sets.append(node_set)
        return node_sets


    def _build_data(self, operands, operations, set_data, READWRITE_LINE=3):
        """
        operands contains a tensor implicitly describing the possible operands for the companion operations.
        the first dimension of this tensor is the index across the READWRITE_LINE, which is when operand
        descriptions transition from being about input to being about output.

        operations is a tensor of shape (num_ops, maximum_operands_per_operation) which establishes the ids
        of the operations via index, and the maximum_operands_per_operation keeps the tensor padded with -1 ids
        """
        num_ops = operations.shape[-1]
        
        # operands should take the form of (N, , )

        data = {
            'operations_types': list(self.solver.operations_types.values()),
            'operation_identities':  list(range(num_ops)),
            'operation_sets': list(operations),
            'node_sets': self.get_node_sets(set_data),
            'input_sets': list(operands[:READWRITE_LINE]),
            'input_masks': list(torch.ones_like(operands[:READWRITE_LINE]), dtype=torch.bool),
            'output_sets': list(operands[READWRITE_LINE:]),
            'output_masks': list(torch.ones_like(operands[READWRITE_LINE:]), dtype=torch.bool),
            'memory_space': list(range(operands.shape[-1])),
            'memory_masks': list(torch.ones_like(range(operands.shape[-1])), dtype=torch.bool),
        }
        return data
...
A = NodeSet(MAX_M, MAX_N, 1),  # A
B = NodeSet(MAX_M, MAX_N, 1),  # B
C = NodeSet(MAX_M, MAX_N, 1),  # C
D = NodeSet(MAX_M, MAX_N, 1),  # D
MATMULFUSEADD_OPERATION_SET = (
    [
        A, B, C, D
    ],

    [
        Operation.get_matmul_fuse(
            [A, B, C], [D]  # MatMulFuseAdd operation
        )
    ]
)
class BatchedTorchParametricSolver(nn.Module):
    def __init__(self, operands, operations, set_data, max_matrix_shape=None, max_total_elements=None, attn_embed_dim=32, attn_layers=1):
        super().__init__()
        self.diagnostics = Diagnostics(self)
        self.diagnostics._build_data(operands, operations, set_data)
    ...
    def from_logits(self, mem_logits_batch, op_tgt_batch, op_src_batch, valid_mask, reinforce=True, matrix_shapes=None, matrix_masks=None, attn_micro=0):
        ...
        self.diagnostics.register_memory_map(sampled_mem_order)
        ...
        mem_conv = F.relu(self.memory_space_conv(self.diagnostics.memory_map))
...
if __name__ == "__main__":
    ...
    # =======================================
    # Integrate symbolic graph analysis
    # =======================================
    operations_list, symbols_set = extract_operations_and_symbols()

    print("\n=== Symbolic Matrix Multiplication Analysis ===")
    print("Symbols (inputs, index and matrix variables):")
    print(sorted(symbols_set))

    print("\nExecution plan by dependency level (operations):")
    for lvl, ops in enumerate(operations_list):
        print(f"Level {lvl}:")
        for op_type, op_str in ops:
            print(f"  - {op_type}: {op_str}")
    ...
    operands, operations = MATMULFUSEADD_OPERATION_SET
    model = BatchedTorchParametricSolver(operands, operations, set_data).to(device)
